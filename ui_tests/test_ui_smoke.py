...
def test_app_loads(page: Page, base_url: str):
    ...
    expect(page.get_by_text("MUIOGO", exact=True)).to_be_visible()
...
def test_parameters_and_settings_routing(page: Page, base_url: str):
    ...
    expect(page.get_by_text("Select Parameter")).to_be_visible()
...
def test_new_case_modal_interaction(page: Page, base_url: str):
    ...
    expect(page.locator("#app-content, .container-fluid").first).to_be_visible()
