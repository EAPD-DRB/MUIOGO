...
export class Message {
    ...
    $("#osy-warning-transparent").html(
      `
                                    <div class="alert alert-warning-osy fade in">
                                        ...
                                        <strong>Warning!</strong> <i style="color:grey">` +
        message +
        `</i>
                                    </div>`,
    );
    ...
    $("#osy-success-transparent").html(
      `
                                    <div class="alert alert-success-osy fade in">
                                        ...
                                        <strong>Success!</strong><i style="color:grey">` +
        message +
        `</i>
                                    </div>`,
    );
  ...
  static infoOsy(message) {
    $("#osy-info-transparent").html(
      `<div class="alert alert-info-osy fade in">
                                        <button class="close" data-dismiss="alert">×</button>
                                        <i class="fa-fw fa fa-info fa-2x info"></i>
                                        <strong>Info!</strong> <i style="color:grey">` +
        message +
        `</i>
                                    </div>`,
    );
  ...
  static dangerOsy(message) {
    $("#osy-danger-transparent").html(
      `<div class="alert alert-danger-osy fade in">
                                        <button class="close" data-dismiss="alert">×</button>
                                        <i class="fa-fw fa fa-exclamation-triangle fa-2x danger"></i>
                                        <strong>Error!</strong> <i style="color:grey">` +
        message +
        `</i>
                                    </div>`,
    );
  ...
  static SmartMessageBoxDDL(cases, init_f) {
    ...
    $.SmartMessageBox(
      {
        title:
          "<i class='fa fa-exclamation-triangle danger'></i>No active Model.",
  ...
  static ddlActivity(cases, init_f) {
    ...
    $.SmartMessageBox(
      {
        title:
          "<i class='fa fa-exclamation-triangle danger'></i>Selected model has no activity defines.",
      ...
      function (ButtonPressed) {
        if (ButtonPressed === "Yes") {
          $.smallBox({
            title: "Confirmation",
            content:
              "<i class='fa fa-clock-o'></i> <i>Technology is deleted!</i>",
            color: "#5384AF",
            iconSmall: "fa fa-check fa-2x fadeInRight animated",
            timeout: 4000,
          });
        ...
        if (ButtonPressed === "No") {
          $.smallBox({
            title: "Confirmation",
            content:
              "<i class='fa fa-clock-o'></i> <i>Deletion is aborted!</i>",
            color: "#C79121",
  ...
  // Enhanced API error methods for Error Boundary integration
  static showAPIError(message, details = null) {
    // Delegate to error boundary if available
    if (typeof window.MUIErrorBoundary !== "undefined") {
      window.MUIErrorBoundary.showErrorToast(message, "error", details);
    } else {
  ...
  static showAPIWarning(message, details = null) {
    if (typeof window.MUIErrorBoundary !== "undefined") {
      window.MUIErrorBoundary.showErrorToast(message, "warning", details);
    } else {
  ...
  static showNetworkError(
    ...
    this.showAPIError(message, { type: "network" });
  }

  static showServerError(
    message = "Server error occurred. Please try again later.",
  ) {
    this.showAPIError(message, { type: "server" });
  }
}
