...
Required qualification is: four source-only gates; application generation and preprocessing; GLPK matrix check; one BASE CBC optimum; then all policy cases, result validation, source-only promotion and byte-identity checks. Runtime ratios are reported, but the scenario-specific COAL_PHASEOUT increase is not judged against a fixed historical ratio because the corrected biomass formulation changes the active policy problem. No failed disposable solver result is copied into the live case.
""", encoding="utf-8")
